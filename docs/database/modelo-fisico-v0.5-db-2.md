...
Alcance de esta versión: corregir y validar el modelo físico `v0.5-db-1` antes de diseñar la transacción `CONFIRMAR VENTA`. No define framework backend, frontend ni aplicación de escritorio.
...
No se refactoriza todo el esquema en db-2 para incluir `business_id` redundante en cada FK compuesta; queda como deuda técnica consciente para evaluar tras validar transacciones críticas.

## 11. Validación real

La intención de db-2 es ejecutar el DDL contra una base temporal vacía y luego correr `database/validation-v0.5-db-2.sql`.

Desde este entorno, al momento de generar el archivo, el puerto `localhost:5432` rechazó conexión y no estaba disponible `psql`. La ejecución real debe realizarse en cuanto el cliente/puerto esté accesible desde el entorno de trabajo.
